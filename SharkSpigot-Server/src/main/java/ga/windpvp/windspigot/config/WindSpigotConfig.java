...
public class WindSpigotConfig {
	...
	private static final String HEADER = "This is the main configuration file for SharkSpigot.\n"
			+ "As you can see, there's tons to configure. Some options may impact gameplay, so use\n"
			+ "with caution, and make sure you know what each option does before configuring.\n" + "\n"
			+ "If you need help with the configuration or have any questions related to SharkSpigot,\n"
	...
	public static void init(File configFile) {
		...
		try {
			WindSpigot.LOGGER.info("Loading SharkSpigot config from " + configFile.getName());
			config.load(CONFIG_FILE);
		} catch (IOException ignored) {
		} catch (InvalidConfigurationException ex) {
			LOGGER.log(Level.ERROR, "Could not load sharkspigot.yml, please correct your syntax errors", ex);
		...
		try {
			...
			LOGGER.warn("Please regenerate your sharkspigot.yml file to prevent this issue! The server will run with the default config for now.");
	...
	static void loadComments() {
		...
		c.addComment("settings.command", "Configuration for SharkSpigot's commands");
		...
		c.addComment("settings.command.ping.enable", "Enables the command \"/ping <player>\" which shows player ping. Users require the permission sharkspigot.command.ping");
		...
		c.addComment("settings.statistics", "Enables WindSpigot statistics. This allows developers to see how many Shark q Spigot servers are running. \nThis has no performance impact and is completely anonymous, but you can opt out of this if you want.");
